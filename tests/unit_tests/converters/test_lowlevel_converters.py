...
def test_converters_namespace_wrappers(monkeypatch: pytest_MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        ...
        types_SimpleNamespace(convert_pytorch_to_onnx=fake_pt),
    )
    monkeypatch.setitem(
        sys_modules,
        "onnx_converter.converters.tensorflow_converter",
        types_SimpleNamespace(convert_tensorflow_to_onnx=fake_tf),
    )
    monkeypatch.setitem(
        sys_modules,
        "onnx_converter.converters.sklearn_converter",
        types_SimpleNamespace(convert_sklearn_to_onnx=fake_sk),
    )
...
def test_pytorch_converter_success(
    ...
    fake_torch = types_SimpleNamespace(
        nn=types_SimpleNamespace(Module=object),
        randn=lambda *shape: ("dummy", shape),
        onnx=types_SimpleNamespace(export=fake_export),
    )
    monkeypatch.setitem(sys_modules, "torch", fake_torch)
    monkeypatch.setitem(sys_modules, "torch.onnx", fake_torch.onnx)
...
def test_pytorch_converter_validation_error(monkeypatch: pytest_MonkeyPatch) -> None:
    """Raise ConversionError when options fail schema validation."""
    fake_torch = types_SimpleNamespace(
        nn=types_SimpleNamespace(Module=object),
        randn=lambda *_: None,
        onnx=types_SimpleNamespace(export=lambda **_: None),
    )
    monkeypatch.setitem(sys_modules, "torch", fake_torch)
    monkeypatch.setitem(sys_modules, "torch.onnx", fake_torch.onnx)

    module = _reload_module("onnx_converter.converters.pytorch_converter")
    with pytest_raises(ConversionError, match="Invalid PyTorch export options"):
...
def test_sklearn_converter_infers_initial_types(
    ...
    fake_data_types = types_SimpleNamespace(FloatTensorType=FakeFloatTensorType)
    monkeypatch.setitem(
        sys_modules,
        "skl2onnx",
        types_SimpleNamespace(convert_sklearn=fake_convert_sklearn),
    )
    monkeypatch.setitem(
        sys_modules,
        "skl2onnx.common",
        types_SimpleNamespace(data_types=fake_data_types),
    )
    ...
    model = types_SimpleNamespace(n_features_in_=4)
...
def test_sklearn_converter_requires_initial_types_when_not_inferable(
    ...
    fake_data_types = types_SimpleNamespace(FloatTensorType=lambda *_: object())
    monkeypatch.setitem(
        sys_modules,
        "skl2onnx",
        types_SimpleNamespace(convert_sklearn=lambda **_: object()),
    )
    monkeypatch.setitem(
        sys_modules,
        "skl2onnx.common",
    ...
    module = _reload_module("onnx_converter.converters.sklearn_converter")
    with pytest_raises(ConversionError, match="Could not infer input types"):
        module.convert_sklearn_to_onnx(model=object(), output_path="x.onnx")


def test_tensorflow_converter_paths(monkeypatch: pytest_MonkeyPatch) -> None:
    ...
    class FakeModelBase:
        ...
        input_shape = (None, 4)
        outputs = [types_SimpleNamespace(name="out:0")]
    ...
    fake_tf = types_SimpleNamespace(
        ...
        keras=types_SimpleNamespace(Model=FakeModelBase),
    )
    fake_tf2onnx = types_SimpleNamespace(
        convert=types_SimpleNamespace(
    ...
    monkeypatch.setitem(sys_modules, "tf2onnx", fake_tf2onnx)
    ...
    with pytest_raises(ValueError, match="Unsupported model type"):
